...
def plot_asynchronous_gap_theory(output_dir: Path) -> None:
    ...
    if crossover_idx > 0:
        ...
        ax.axvline(
            ...
            linestyle="--",
            alpha=0.7,
            label="Critical Crossover Point",
        )
    ...
    ax.set_title(
        "The Asynchronous Gap: Exponential Technology vs. " "Logarithmic Maturity",
        fontsize=15,
        fontweight="bold",
    )
    ...
    fig.tight_layout()
    fig.savefig(
        output_dir / "asynchronous_gap_theory.png",
        dpi=300,
        bbox_inches="tight",
...
def plot_population_dynamics(tick_df: pd.DataFrame, output_dir: Path) -> None:
    ...
    ax.fill_between(
        tick_df["tick"], tick_df["active_agents"], alpha=0.3, color=COLORS["active"]
    )
    ax.plot(
        tick_df["tick"],
        tick_df["active_agents"],
        color=COLORS["active"],
        linewidth=1.5,
        label="Active",
    )
    ...
    if "collapsed_count" in tick_df.columns:
        ax.plot(
            tick_df["tick"],
            tick_df["collapsed_count"],
            color=COLORS["collapsed"],
            linewidth=1.5,
            linestyle="--",
            label="Collapsed (cumulative)",
        )

    if "transcended_count" in tick_df.columns:
        ax.plot(
            tick_df["tick"],
            tick_df["transcended_count"],
            color=COLORS["transcended"],
            linewidth=1.5,
            linestyle="-.",
            label="Transcended (cumulative)",
        )
    ...
    fig.tight_layout()
    fig.savefig(
        output_dir / "population_dynamics.png",
        dpi=300,
        bbox_inches="tight",
        facecolor=COLORS["background"],
    )
...
def plot_state_vector_evolution(tick_df: pd.DataFrame, output_dir: Path) -> None:
    ...
    fig, axes = plt.subplots(
        3, 1, figsize=(14, 12), sharex=True, facecolor=COLORS["background"]
    )
    ...
    axes[0].set_title(
        "State Vector Evolution Over Time", fontsize=14, fontweight="bold"
    )

    fig.tight_layout()
    fig.savefig(
        output_dir / "state_vector_evolution.png",
        dpi=300,
        bbox_inches="tight",
        facecolor=COLORS["background"],
    )
...
def plot_collapse_analysis(collapse_df: pd.DataFrame, output_dir: Path) -> None:
    ...
    ax.set_title(
        "State Vectors at Moment of Civilizational Collapse",
        fontsize=14,
        fontweight="bold",
    )
    ax.legend(fontsize=10, framealpha=0.3)
    ...
    fig.tight_layout()
    fig.savefig(
        output_dir / "collapse_scatter.png",
        dpi=300,
        bbox_inches="tight",
        facecolor=COLORS["background"],
    )
    ...
    ax.set_title(
        "Temporal Distribution of Collapse Events", fontsize=14, fontweight="bold"
    )
    ax.grid(True, alpha=0.2, color=COLORS["grid"])

    fig.tight_layout()
    fig.savefig(
        output_dir / "collapse_histogram.png",
        dpi=300,
        bbox_inches="tight",
        facecolor=COLORS["background"],
    )
...
def plot_spatial_distribution(
    ...
    for state, color in state_colors.items():
        ...
        if mask.any():
            ...
            ax.scatter(
                subset["pos_x"], subset["pos_y"], c=color, s=10, alpha=0.5, label=state
            )

    ax.set_xlabel("X", fontsize=12)
    ax.set_ylabel("Y", fontsize=12)
    ax.set_title(
        "Final Spatial Distribution of Civilizations", fontsize=14, fontweight="bold"
    )
    ...
    fig.tight_layout()
    fig.savefig(
        output_dir / "spatial_distribution.png",
        dpi=300,
        bbox_inches="tight",
        facecolor=COLORS["background"],
    )
...
def main() -> None:
    ...
    output_dir = (
        Path(args.output_dir) if args.output_dir else Path(args.data_dir) / "figures"
    )
