...
BUG FIXES in this version:
  1. DARK_LAYOUT excludes 'xaxis'/'yaxis' — axes via update_xaxes/update_yaxes
     only (eliminates 'multiple values for keyword argument' TypeError).
  2. selectbox bound to key="run_selector" — reactive cache invalidation.
...
DARK_LAYOUT = dict(
    ...
    colorway=["#00d2ff","#7b2ff7","#ff6b6b","#ffd93d","#6bcb77","#ff8fab","#4cc9f0","#f72585"],
)

# Axis styling applied via update_xaxes / update_yaxes — never via layout kwargs.
DARK_AXES = dict(gridcolor="#30363d", zerolinecolor="#30363d", linecolor="#30363d")

COLORS = dict(
    energy="#00d2ff", tribalism="#ff6b6b", collectivism="#7b2ff7",
    transcended="#6bcb77", collapsed="#ff6b6b", active="#00d2ff", gap="#ffd93d",
)
...
# ── Run discovery (not cached — must be fresh on every re-run) ────────────────
def discover_runs(resolved_base: Path) -> list:
    """
    Strictly returns subdirectory NAMES found inside resolved_base/runs/.
    Only directories (not files) are listed. Sorted newest-first.
    Orphaned files directly in resolved_base are ignored here
    and reported separately via has_orphaned_data().
    """
    ...
    return sorted(
        ...
        reverse=True,  # timestamp-prefixed names sort lexicographically = chronologically
    )
...
def orphan_cleanup_suggestion(resolved_base: Path) -> str:
    ...
    orphans = sorted(
        f.name
        for ext in ("*.csv", "*.json")
        for f in resolved_base.glob(ext)
    )
...
# ── Data loaders — cached per absolute run directory string ───────────────────
# Cache key = run_dir string. Different run → different string → cache miss →
...
def load_tick_history(run_dir: str) -> pd.DataFrame:
    for fname, as_json in [("tick_history.csv", False), ("tick_history.json", True)]:
        ...
        if p.exists():
            return pd.read_csv(p) if not as_json else pd.DataFrame(json.loads(p.read_text()))
    return pd.DataFrame()
...
base_data_dir_raw = st.sidebar.text_input(
    "Base Data Directory", value="../data",
    help="Relative or absolute path to the directory containing the runs/ folder.",
)

# Resolve to absolute path — eliminates ../ ambiguity regardless of cwd.
...
if has_orphaned_data(resolved_base):
    with st.sidebar.expander("⚠️ Orphaned files detected", expanded=False):
        st.warning(orphan_cleanup_suggestion(resolved_base))

# ── Empty state ───────────────────────────────────────────────────────────────
if not available_runs:
    st.markdown('<div class="header-gradient">CAT Simulation Dashboard</div>',
                unsafe_allow_html=True)
    st.markdown('<div class="subheader">Cosmobiological Asynchrony Theory — '
                'Fermi Paradox Agent-Based Model</div>', unsafe_allow_html=True)
    st.info(
...
# On every script re-run, `selected_run` is the widget's CURRENT value —
# not a stale default — guaranteeing the data loaders receive the correct path.
...
page = st.sidebar.radio(
    "View",
    ["Overview", "State Vectors", "Collapse Analysis",
     "Spatial Distribution", "Asynchronous Gap", "Configuration"],
)

# ── Load data for the selected run ────────────────────────────────────────────
# selected_run_dir changes every time selected_run changes.
# @st.cache_data keys on this string — different string = fresh file read.
...
if page == "Overview":
    ...
    st.markdown(
        ...
        f' &nbsp;|&nbsp; {len(available_runs)} run(s) available</div>',
        unsafe_allow_html=True,
    )
    ...
    with c1:
        st.markdown(metric_html("Total Collapses",
            f"{int(last.get('collapsed_count', 0)):,}"), unsafe_allow_html=True)
    with c2:
        st.markdown(metric_html("Transcended",
            f"{int(last.get('transcended_count', 0)):,}"), unsafe_allow_html=True)
    with c3:
        st.markdown(metric_html("Still Active",
            f"{int(last.get('active_agents', 0)):,}"), unsafe_allow_html=True)
    with c4:
        st.markdown(metric_html("Collapse Rate",
            f"{collapse_rate:.1f}%"), unsafe_allow_html=True)

    st.markdown("---")

    fig = go.Figure()
    fig.add_trace(go.Scatter(x=tick_df["tick"], y=tick_df["active_agents"],
        name="Active", fill="tozeroy", line=dict(color=COLORS["active"], width=2)))
    if "collapsed_count" in tick_df.columns:
        fig.add_trace(go.Scatter(x=tick_df["tick"], y=tick_df["collapsed_count"],
            name="Collapsed (cumulative)",
            line=dict(color=COLORS["collapsed"], width=2, dash="dot")))
    if "transcended_count" in tick_df.columns:
        fig.add_trace(go.Scatter(x=tick_df["tick"], y=tick_df["transcended_count"],
            name="Transcended (cumulative)",
            line=dict(color=COLORS["transcended"], width=2, dash="dash")))
    dark_fig(fig, height=500, title="Population Dynamics Over Time",
             xaxis_title="Simulation Tick", yaxis_title="Agent Count")
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    fig = make_subplots(
        ...
        subplot_titles=(
            "Mean Energy  E(t) = E₀·e^(r·t)",
            "Mean Tribalism  T(t) = T₀·max(0, 1−α·ln(1+t))",
            "Mean Collectivism  C(t) = clamp(C₀+δ·t, 0, 1)",
        ),
    )
    fig.add_trace(go.Scatter(x=tick_df["tick"], y=tick_df["mean_energy"],
        name="Energy", line=dict(color=COLORS["energy"], width=2)), row=1, col=1)
    fig.add_trace(go.Scatter(x=tick_df["tick"], y=tick_df["mean_tribalism"],
        name="Tribalism", line=dict(color=COLORS["tribalism"], width=2)), row=2, col=1)
    fig.add_trace(go.Scatter(x=tick_df["tick"], y=tick_df["mean_collectivism"],
        name="Collectivism", line=dict(color=COLORS["collectivism"], width=2)), row=3, col=1)
    ...
    if "max_energy" in tick_df.columns:
        ...
        fig2 = go.Figure()
        fig2.add_trace(go.Scatter(x=tick_df["tick"], y=tick_df["max_energy"],
            name="Max E", line=dict(color=COLORS["gap"], width=2)))
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    with col1:
        ...
        st.plotly_chart(fig, use_container_width=True)
    with col2:
        fig = px.histogram(collapse_df, x="tick", nbins=50,
    ...
    fig = px.scatter(collapse_df, x="energy", y="tribalism",
                     color="collapse_type", size="collectivism",
    ...
    st.dataframe(
        collapse_df[["energy","tribalism","collectivism"]].describe().style.format("{:.4f}"),
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    fig = px.scatter(agents_df, x="pos_x", y="pos_y", color="state",
                     size="energy",
                     title="Final Agent Positions",
                     color_discrete_map={
                         "Nascent":"#8b949e","Evolving":"#00d2ff",
                         "Transcended":"#7b2ff7","Collapsed":"#ff6b6b",
                     },
    ...
    if not collapse_df.empty:
        ...
        fig = px.density_heatmap(collapse_df, x="pos_x", y="pos_y",
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=x, y=e_curve, name="E = e^(0.15·t)",
        line=dict(color=COLORS["tribalism"], width=3)))
    fig.add_trace(go.Scatter(x=x, y=t_curve, name="Maturity = 2.5·ln(t+1)",
        line=dict(color=COLORS["energy"], width=3)))

    crossover = int(np.argmax(e_curve > t_curve))
    if crossover:
        fig.add_vrect(x0=float(x[crossover]), x1=float(x[-1]),
                      fillcolor="#ff6b6b", opacity=0.08,
                      annotation_text="ASYNCHRONOUS GAP",
                      annotation_position="top left",
                      annotation_font_color="#ff6b6b")
        fig.add_vline(x=float(x[crossover]), line_color=COLORS["gap"],
                      line_dash="dash", opacity=0.7)

    # Apply base dark layout — no yaxis kwarg here.
    ...
    fig.update_xaxes(**DARK_AXES)
    # Log scale applied via update_yaxes — no collision with DARK_LAYOUT.
    fig.update_yaxes(**DARK_AXES, type="log")
    st.plotly_chart(fig, use_container_width=True)

    if "mean_async_gap" in tick_df.columns:
        st.markdown("### Simulated Mean Gap  G(t) = E / (1−T+ε)")
        fig2 = go.Figure()
