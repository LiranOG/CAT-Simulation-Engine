...
if incomplete_runs:
    with st.sidebar.expander(f"⚠️ {len(incomplete_runs)} incomplete run(s)", expanded=False):
        st.warning(
...
st.sidebar.markdown(f'<div class="run-badge">{selected_run}</div>', unsafe_allow_html=True)
...
# -----------------------------------------------------------------------------
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=x, y=e_curve, name="E = exp(0.15 * t)",
